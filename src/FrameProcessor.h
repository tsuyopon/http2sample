...
public:
	static int readFrameLoopServer(ConnectionState* con_state, SSL* ssl);
	static int readFrameLoop(ConnectionState* con_state, SSL* ssl, const std::vector<std::map<std::string, std::string>> headermap);
	// write
...
private:
	...
	static void _rcv_window_update_frame(SSL* ssl, unsigned int &streamid, unsigned int &payload_length, unsigned char* &p);
